...
def fetch_data(year, use_advanced=False):
...
def fetch_salaries(year):
    ...
    salaries_table = soup.find_all("table",
    ...
    player_data = [[td.getText().strip() for td in data_rows[i].findAll('td')] for i in range(len(data_rows))]
